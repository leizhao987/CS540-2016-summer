/****************************************************************
 * Match.java Do not modify this file! This file actually plays the game.
 * -----------------------------------------------------------------------------
...
public class Match {
	...
	/**
	 * Creates a new match object. Both player classes should extend the
	 * abstract Player class. The classes should be on the classpath at run time
	 *
	 * @param player1
	 *            the name of the class representing player1
	 * @param player2
	 *            the name of the class representing player2
	 * @param numStones
	 *            the initial number of stones in each bin, must be >= 1
	 * @param thinkTime
	 *            the maximum allowable think time per move in seconds.
	 *
	 ...
	 * until the game ends.
	 * <p>
	 * Each move is timed with thinkTime. A player is allowed to run
	 * searchMove() for this amount of time. After which, the move is acquired
	 * using the getMove() method.
	 *
	...
	public int play() {
		...
		while (!gameContext.gameOver()) {
			...
			try {
				synchronized (mutex) {
					MovingThread movingThread = new MovingThread(currentMover, new GameState(gameContext), mutex,
							timeout);
					movingThread.start(); // schedule the moving thread
					mutex.wait(timeout); // go to sleep for the timeout or until
											// the move returns

					Thread.sleep(500); // sleep for half a second to allow for
										// cleanup
					if (movingThread.isAlive()) {
						movingThread.stop(); // kill the moving thread if it is
												// still going
			...
			if (gameContext.illegalMove(move)) { // the player chose an illegal
													// move, lose immediately
			...
			Mancala.gui.applyMove(move, player1Move, new GameState(gameContext)); // update
																					// the
																					// GUI
			playAgain = gameContext.applyMove(move); // effect the move on the
														// board
		}
	...
	/**
	 * Takes care of executing the move in a separate thread so that we can
	 * timeout if the player takes too long.
	 */
	private static class MovingThread extends Thread {
		...
		public void run() {
			...
			moveResult = 0;
			try {
				synchronized (mutex) { // this is just to ensure that the player
										// doesn't
				...
				synchronized (mutex) {
					mutex.notify(); // wake up the match if we finished before
									// the time
				}
			} catch (Exception e) {
